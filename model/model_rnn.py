...
class Dual_Path_RNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels,
                 ...
                 bidirectional=False, num_layers=4, K=250, num_spks=2):
        ...
        # self.output = nn.Conv1d(out_channels, out_channels, 1)
        self.output_gate = nn.Sequential(nn.Conv1d(out_channels, out_channels, 1),
                                         nn.Sigmoid()
                                         )

        # self.output_gate = nn.Conv1d(out_channels, out_channels, 1)
    def forward(self, x):
        '''
           x: [B, N, L]
