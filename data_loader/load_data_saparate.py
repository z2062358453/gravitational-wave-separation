...
def get_val_batch_for_denoise_task(batch_size,sample_length,snr_low_list,snr_high_list, peak_range,freq, snr_change_time, datapath):
    ...
    for noise_E1, signal_E1_1, signal_E1_2, noise_E2, signal_E2_1, signal_E2_2, noise_E3, signal_E3_1, signal_E3_2 in my_denoising_iter:
        ...
        # print(strain.shape)
        if count == 1:
            batch_x = (noise_E1 + signal_E1_1 + signal_E1_2) / np.max(noise_E1 + signal_E1_1 + signal_E1_2)
            batch_y_1 = (signal_E1_1) / np.max(signal_E1_1)
            batch_y_2 = (signal_E1_2) / np.max(signal_E1_2)
        else:
            batch_x = np.concatenate(
                (batch_x, (noise_E1 + signal_E1_1 + signal_E1_2) / np.max(noise_E1 + signal_E1_1 + signal_E1_2)))
            batch_x = np.concatenate(
                (batch_x, (noise_E2 + signal_E2_1 + signal_E2_2) / np.max(noise_E2 + signal_E2_1 + signal_E2_2)))
            batch_x = np.concatenate(
                (batch_x, (noise_E3 + signal_E3_1 + signal_E3_2) / np.max(noise_E3 + signal_E3_1 + signal_E3_2)))
            batch_y_1 = np.concatenate((batch_y_1, (signal_E1_1) / np.max(signal_E1_1)))
            batch_y_2 = np.concatenate((batch_y_2, (signal_E1_2) / np.max(signal_E1_2)))
            batch_y_1 = np.concatenate((batch_y_1, (signal_E2_1) / np.max(signal_E2_1)))
            batch_y_2 = np.concatenate((batch_y_2, (signal_E2_2) / np.max(signal_E2_2)))
            batch_y_1 = np.concatenate((batch_y_1, (signal_E3_1) / np.max(signal_E3_1)))
            batch_y_2 = np.concatenate((batch_y_2, (signal_E3_2) / np.max(signal_E3_2)))
        if count == batch_size:
...
# def get_train_batch_iter_shuffle_epoch(batch_size, sample_length, snr_low_list, snr_high_list, peak_range, freq, snr_change_time,datapath):
#   while True:
#       index_list=list(range(len(datapath)))
...
#       batch_y_1 = []
#       batch_y_2 = []
#       for _, signal_E1_1, signal_E1_2, _, signal_E2_1, signal_E2_2, _, signal_E3_1, signal_E3_2 in my_denoising_iter:
#         # print(noise_E1.shape)
#         # print(strain.shape)
#         if count == 1:
#             batch_x = (signal_E1_1 + signal_E1_2) / np.max(signal_E1_1 + signal_E1_2)
#             batch_y_1 = (signal_E1_1) / np.max(signal_E1_1)
#             batch_y_2 = (signal_E1_2) / np.max(signal_E1_2)
#         else:
#             batch_x = np.concatenate(
#                 (batch_x, (signal_E1_1 + signal_E1_2) / np.max(signal_E1_1 + signal_E1_2)))
#             batch_x = np.concatenate(
#                 (batch_x, (signal_E2_1 + signal_E2_2) / np.max(signal_E2_1 + signal_E2_2)))
#             batch_x = np.concatenate(
#                 (batch_x, (signal_E3_1 + signal_E3_2) / np.max(signal_E3_1 + signal_E3_2)))
#             batch_y_1 = np.concatenate((batch_y_1, (signal_E1_1) / np.max(signal_E1_1)))
#             batch_y_2 = np.concatenate((batch_y_2, (signal_E1_2) / np.max(signal_E1_2)))
#             batch_y_1 = np.concatenate((batch_y_1, (signal_E2_1) / np.max(signal_E2_1)))
#             batch_y_2 = np.concatenate((batch_y_2, (signal_E2_2) / np.max(signal_E2_2)))
...
#             yield (batch_x.reshape(-1, sample_length * freq, 1), batch_y_1.reshape(-1, sample_length * freq, 1),
#                    batch_y_2.reshape(-1, sample_length * freq, 1))
#             del batch_x, batch_y_1, batch_y_2, signal_E1_1, signal_E1_2, signal_E2_1, signal_E2_2, signal_E3_1, signal_E3_2
#             gc.collect()
#         count = count + 3
#         if count > batch_size:
#             count = 1
def get_train_batch_for_denoise_task(batch_size, sample_length, snr_low_list, snr_high_list, peak_range, freq, snr_change_time,datapath):
      my_denoising_iter = detach_sample_cut_iter(sample_length, snr_low_list, snr_high_list, peak_range, freq,
                                               snr_change_time, datapath)
      count = 1
      batch_x = []
...
def get_train_batch_for_end2end(batch_size, sample_length, snr_low_list, snr_high_list, peak_range, freq,
                                     snr_change_time, datapath):
    my_denoising_iter = detach_sample_cut_iter(sample_length, snr_low_list, snr_high_list, peak_range, freq,
                                               snr_change_time, datapath)
    count = 1
    ...
    for noise_E1, signal_E1_1, signal_E1_2, noise_E2, signal_E2_1, signal_E2_2, noise_E3, signal_E3_1, signal_E3_2 in my_denoising_iter:
        ...
        if count > batch_size:
            count = 1
def get_val_batch_for_end2end(batch_size, sample_length, snr_low_list, snr_high_list, peak_range, freq,
                                     snr_change_time, datapath):
    my_denoising_iter = detach_sample_cut_iter(sample_length, snr_low_list, snr_high_list, peak_range, freq,
                                               snr_change_time, datapath)
    count = 1
...
# batch_size =16
# generator = get_train_batch_for_denoise_task(batch_size=batch_size, sample_length=sample_length, snr_low_list=snr_low_list,
