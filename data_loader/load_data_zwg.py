...
def get_train_batch_iter(batch_size,sample_length,snr_low_list,snr_high_list, peak_range,freq, snr_change_time, datapath):
    ...
    for noise_E1, signal_E1_1, signal_E1_2, noise_E2, signal_E2_1, signal_E2_2, noise_E3, signal_E3_1, signal_E3_2 in my_denoising_iter:
        ...
        if count == 1:
            ...
            batch_y = (signal_E1_1 + signal_E1_2) / np.max(signal_E1_1 + signal_E1_2)
        else:
            ...
            batch_y = np.concatenate((batch_y, (signal_E1_1 + signal_E1_2) / np.max(signal_E1_1 + signal_E1_2)))
            batch_y = np.concatenate((batch_y, (signal_E2_1 + signal_E2_2) / np.max(signal_E2_1 + signal_E2_2)))
            batch_y = np.concatenate((batch_y, (signal_E3_1 + signal_E3_2) / np.max(signal_E3_1 + signal_E3_2)))
        if count == batch_size:
...
# batch_size = 16
# generator = get_train_batch_iter_shuffle_epoch(batch_size=batch_size, sample_length=sample_length, snr_low_list=snr_low_list,
